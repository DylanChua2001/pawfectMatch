...
const TrainingPackagesList = () => {
  ...
  const fetchTrainingPackages = async () => {
    try {
      const response = await fetch('https://pawfect-match-three.vercel.app/api/trainPack/getAllTrainingPack');
  ...
  const handleAddToCart = async (trainingPackage) => {
    try {
      await axios.put(`https://pawfect-match-three.vercel.app/api/cart/addCart/${userId}/add/${trainingPackage.train_id}`);
